...
if __name__ == "__main__":
    ...
    for pictures_dir in pictures_dirs:
        ...
        for picture in picture_files:
            ...
            statistics.append([ picture.name, radius, center, radiusErrorPercent, centerErrorXPercent, centerErrorYPercent])


    print("Picture Name, Computed Radius, Center, Radius Error Percent, Center X Error Percent, Center Y Error Percent")
    for stat in statistics:
        print(f"{stat[0]}, {stat[1]}, {stat[2]}, {stat[3]:.2f}%, {stat[4]:.2f}%, {stat[5]:.2f}%")   
    mean_radius_error = sum(stat[3] for stat in statistics) / len(statistics)
    mean_center_x_error = sum(stat[4] for stat in statistics) / len(statistics)
    mean_center_y_error = sum(stat[5] for stat in statistics) / len(statistics)
